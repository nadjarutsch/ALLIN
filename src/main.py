...
def main(cfg: DictConfig):
    ...
    for seed in seeds:
        config['seed'] = seed
        run = wandb.init(project="idiod", entity="nadjarutsch", group='depcon extensive experiments clustering & causal discovery', notes='', tags=['kmeans', 'jci', 'pc', 'depcon'], config=config, reinit=True)
        with run:
            # generate data
