...
def get_likelihood(batch: torch.Tensor, preds: torch.Tensor) -> torch.Tensor:
    mean, var = preds[..., :int(batch.shape[-1])], torch.exp(preds[..., int(batch.shape[-1]):])
  #  dists = torch.distributions.normal.Normal(mean, std)
  #  log_l = dists.log_prob(batch)
    loss = nn.GaussianNLLLoss(reduction='none')
    return torch.exp(-loss(mean, batch, var))
