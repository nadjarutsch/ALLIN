...
def nll(preds, x, eps=0.00001):
    return - log_normal(x, preds, eps)
   
    
def log_normal(x, preds, eps=0.00001):
    ...
    def forward(self, x, adj_matrix=None):
        for l in self.layers:
            if isinstance(l, (EmbedLayer, InputMask)):
                mask = adj_matrix[None].expand(x.shape[0], -1, -1)
                x = l(x, mask=mask)
            else:
                x = l(x)
        return x
...
class GaussianNoiseModel(nn.Module):
    ...
    def __init__(self, num_vars, hidden_dims):
        ...
        self.condition_mlp = MultivarMLP(input_dims=num_vars*2,  # Times two because of the mask
                                         ...
                                         pre_layers=InputMask(None, concat_mask=True)) # Note: changed from True
        
        self.bias = nn.Parameter(torch.zeros(num_vars)) # std for each variable, independent of variable value

    def forward(self, x, mask):
        y = self.condition_mlp(x, mask)
        y = torch.cat((y, self.bias[None,:,None].expand_as(y)), dim=-1)
